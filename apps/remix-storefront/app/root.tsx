...
// CSS bundle - import normally, it will be undefined if not available
...
import { detectChannel } from '~/utils/channel-detection';
...
export const links: LinksFunction = () => {
  ...
  const vendureUrl = typeof process !== 'undefined' 
    ? process.env.VENDURE_API_URL 
    : 'https://readonlydemo.vendure.io';
  if (vendureUrl) {
    try {
      const url = new URL(vendureUrl);
      links.push({ rel: 'preconnect', href: url.origin });
      links.push({ rel: 'dns-prefetch', href: url.origin });
    } catch {
      // Invalid URL, skip
    }
  }
...
export async function loader({ request, params, context }: DataFunctionArgs) {
  ...
  const channelCode = detectChannel(request);
...
export default function App() {
  ...
  const { collections, channelCode } = loaderData;
  ...
  const gaId = channelCode === 'ca' ? GA_CA_ID : GA_US_ID;
  ...
  return (
    <html lang={locale} dir={i18n.dir()} id="app">
      ...
      <body className="bg-navy-dark text-white min-h-screen">
        <Header
