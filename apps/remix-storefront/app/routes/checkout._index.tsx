...
export default function CheckoutShipping() {
  ...
  const defaultShippingIndex = addresses.findIndex(
    (addr) => addr.defaultShippingAddress,
  );

  // Initialize selectedAddressIndex with default address
  useEffect(() => {
    if (
      isSignedIn &&
      addresses.length > 0 &&
      ...
      setSelectedAddressIndex(defaultShippingIndex);
      // Auto-submit default address if no shipping address is set
      if (!shippingAddress?.streetLine1) {
        const timer = setTimeout(
          () => submitSelectedAddress(defaultShippingIndex),
          100,
        );
  ...
  return (
    <div>
      ...
      <div className="mt-10 border-t border-gray-200 pt-10">
        ...
        <CouponCodeInput
          appliedCouponCodes={(activeOrder as any)?.couponCodes || []}
          onCouponApplied={() => {
