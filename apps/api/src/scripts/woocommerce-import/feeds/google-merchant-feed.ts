...
export interface GoogleMerchantProduct {
  ...
  price: string; // Format: "179.14 USD"
  brand: string;
  condition: 'new' | 'used' | 'refurbished';
  mpn?: string; // Manufacturer Part Number (SKU)
  gtin?: string; // UPC/GTIN
  product_type?: string; // Category hierarchy
  google_product_category?: string; // Google's category code
  shipping?: {
    country: string;
    service: string;
    price: string;
...
/**
 * Map Vendure product to Google Merchant format
 */
export function mapProductToGoogleMerchant(
  ...
  return {
    ...
    condition: 'new',
    mpn: variant.sku,
    gtin: (variant.customFields as any)?.upc,
    product_type: categoryPath,
    google_product_category: googleCategory,
    shipping: {
      country: channel === 'us' ? 'US' : 'CA',
      service: 'Standard',
      price: `0 ${currency}`, // Free shipping
    },
  };
}

/**
 * Map category to Google product category code
 */
function mapToGoogleCategory(categoryPath: string): string {
  const path = categoryPath.toLowerCase();
  
  // Home & Garden > Yard & Garden > Irrigation
  if (path.includes('irrigation')) {
    return '1179'; // Home & Garden > Yard & Garden > Irrigation
  }
  
  // Home & Garden > Lighting
  if (path.includes('lighting') || path.includes('luminaire')) {
    return '532'; // Home & Garden > Lighting
  }
  
  // Default
  return '1179'; // Home & Garden > Yard & Garden
}
