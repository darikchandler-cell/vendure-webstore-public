...
// Try multiple token options
const TOKEN_OPTIONS = [
  process.env.US_CHANNEL_TOKEN,
  'us-channel-token',
  'e8feb84eb1c9a971babd442996f62ed2',
  null, // Try without token (public access)
].filter(Boolean);
...
async function checkProductCountWithToken(token) {
    ...
    const req = client.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          const totalItems = result.data?.products?.totalItems;
          if (totalItems !== undefined) {
            resolve(totalItems);
          } else {
            reject(new Error('No totalItems in response'));
          }
        } catch (error) {
          reject(error);
        }
...
async function checkProductCount() {
  ...
  // Try each token option
  for (let i = 0; i < TOKEN_OPTIONS.length; i++) {
    const token = TOKEN_OPTIONS[i];
    const tokenLabel = token || '(no token)';
    
    try {
      console.log(`Trying with token: ${tokenLabel}...`);
      const totalItems = await checkProductCountWithToken(token);
      ...
      if (i === TOKEN_OPTIONS.length - 1) {
        // Last attempt failed
        console.error(`❌ All token options failed. Last error: ${error.message}`);
        throw error;
      }
      // Try next token
      continue;
